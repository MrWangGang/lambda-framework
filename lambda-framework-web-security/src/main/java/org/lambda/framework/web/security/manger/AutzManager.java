...
import org.lambda.framework.web.security.contract.Contract;
...
public abstract class AutzManager implements ReactiveAuthorizationManager<AuthorizationContext>,AutzVerify {
    ...
    @Value("${lambda.security.url_autz:"+ Contract.LAMBDA_SECURITY_URL_AUTZ_ALL+"}")
    public void setUrlAutz(String urlAutz) {
        if(Contract.LAMBDA_SECURITY_URL_AUTZ_ALL.equals(urlAutz) || Contract.LAMBDA_SECURITY_URL_AUTZ_MAPPING.equals(urlAutz)) {
    ...
    public Mono<AuthorizationDecision> check(Mono<Authentication> authentication, AuthorizationContext authorizationContext) {
         ...
         return authManager.authenticate(authorizationContext).flatMap(auth->{
            ...
            return ReactiveRedisOperation.build(securityAutzRedisTemplate).get(currentPath)
                        .onErrorResume(e->Mono.just(Contract.LAMBDA_SECURITY_EMPTY_STR))
                        .defaultIfEmpty(Contract.LAMBDA_SECURITY_EMPTY_STR)
                        .flatMap(currentPathAutzTree->{
                            if(StringUtils.isBlank(currentPathAutzTree.toString())){
                                //如果路径权限树为空
                                if(Contract.LAMBDA_SECURITY_URL_AUTZ_ALL.equals(urlAutz)){
                                    //配置了所有的经过认证都需要授权
                                    return Mono.error(new EventException(EA00000001));
                                }
                                if(Contract.LAMBDA_SECURITY_URL_AUTZ_MAPPING.equals(urlAutz)){
