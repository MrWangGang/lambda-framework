...
public abstract class AbstractReactiveRedissonConfig {
    ...
    protected ReactiveRedissonBuilder redissonConnectionFactory(){
    ...
    private ReactiveRedissonBuilder single(){
        //密码不需要校验
    ...
    private ReactiveRedissonBuilder masterSlave(){
        //密码不需要校验
        Assert.verify(this.host(),ES_LOCK_REDISSON_000);
        Assert.verify(this.database(),ES_LOCK_REDISSON_006);
        Config config = new Config();
    ...
    private ReactiveRedissonBuilder sentinel(){
        //密码不需要校验
        Assert.verify(this.host(),ES_LOCK_REDISSON_000);
        Assert.verify(this.database(),ES_LOCK_REDISSON_006);
    ...
    private ReactiveRedissonBuilder cluster(){
        //密码不需要校验
        Assert.verify(this.host(),ES_LOCK_REDISSON_000);
        Config config = new Config();
