...
public interface ReactiveUnifyPagingRepositoryOperation<Entity> {
    ...
    default <Page extends Paging,Condition>Mono<Paged> find(Page page, Condition condition,UnifyPagingOperation<Condition> operation){
        return this.find(page, condition, Sort.unsorted(),operation);
    }

    default <Page extends Paging,Condition>Mono<Paged> find(Page page, Condition condition, Sort sort, UnifyPagingOperation<Condition> operation){
        ...
        return Mono.zip(
                        ...
                        operation.query(pageRequest,condition).collectList() // 查询数据
                )
                .flatMap(tuple -> {
