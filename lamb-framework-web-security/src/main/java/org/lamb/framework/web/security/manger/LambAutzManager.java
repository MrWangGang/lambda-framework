...
import org.lamb.framework.web.security.contract.LambSecurityContract;
...
public abstract class LambAutzManager implements ReactiveAuthorizationManager<AuthorizationContext>,LambSecurityAutzVerify {
    ...
    @Value("${lamb.security.url_autz:"+ LambSecurityContract.LAMB_SECURITY_URL_AUTZ_ALL+"}")
    public void setUrlAutz(String urlAutz) {
        if(LambSecurityContract.LAMB_SECURITY_URL_AUTZ_ALL.equals(urlAutz) || LambSecurityContract.LAMB_SECURITY_URL_AUTZ_MAPPING.equals(urlAutz)) {
    ...
    public Mono<AuthorizationDecision> check(Mono<Authentication> authentication, AuthorizationContext authorizationContext) {
         ...
         return lambAuthManager.authenticate(authorizationContext).flatMap(auth->{
            ...
            return LambReactiveRedisOperation.build(lambAutzRedisTemplate).get(currentPath)
                        .onErrorResume(e->Mono.just(LambSecurityContract.LAMB_SECURITY_EMPTY_STR))
                        .defaultIfEmpty(LambSecurityContract.LAMB_SECURITY_EMPTY_STR)
                        .flatMap(currentPathAutzTree->{
                            if(StringUtils.isBlank(currentPathAutzTree.toString())){
                                //如果路径权限树为空
                                if(LambSecurityContract.LAMB_SECURITY_URL_AUTZ_ALL.equals(urlAutz)){
                                    //配置了所有的经过认证都需要授权
                                    return Mono.error(new LambEventException(EA00000001));
                                }
                                if(LambSecurityContract.LAMB_SECURITY_URL_AUTZ_MAPPING.equals(urlAutz)){
