package org.lamb.framework.sub.guid;
...
public class LambGUIDFactory {
    ...
    @Value("${lamb.guid.datacenter_id:-1}")
    public void setDatacenterId(Long datacenterId) {
        if(datacenterId == null || datacenterId == -1L){
            throw new LambEventException(ES00000038);
        }
        this.datacenterId = datacenterId;
    }
    @Value("${lamb.guid.machine_id:-1}")
    public void setMachineId(Long machineId) {
        if(machineId == null || machineId == -1L){
            throw new LambEventException(ES00000039);
        }
    ...
    public String GUID(){
         try {
             return String.valueOf(nextId());
                ...
                throw new LambEventException(ES00000037);
         }
    }
    //产生下一个ID
    private synchronized long nextId() {
        ...
        if (currStmp < lastStmp) {
            throw new LambEventException(ES00000036);
        }
