...
public class AbstractParam {
    public void verify(){
        ...
        if(StringUtils.isBlank(openAiApiKey))throw new EventException(EAI0000004);
        if(StringUtils.isBlank(userId))throw new EventException(EAI0000005);
        if(!Contract.verify(userId, uniqueParam))throw new EventException(EAI00000008);
        //配额校验
    ...
    public AbstractParam(Integer n, String prompt, Long quota, Integer maxTokens, Long timeOut, String openAiApiKey, String userId, UniqueParam uniqueParam) {
        this.n = n;
        this.prompt = prompt;
        this.quota = quota;
