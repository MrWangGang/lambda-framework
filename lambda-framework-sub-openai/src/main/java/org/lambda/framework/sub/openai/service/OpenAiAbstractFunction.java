...
import org.lambda.framework.sub.openai.OpenAiModelEnum;
...
public interface OpenAiAbstractFunction {
    ...
    public default boolean limitVerifyByModel(OpenAiModelEnum model, Long quota, Integer maxtoken, Integer promptTokens) {
        if(model.getModel().equals(TURBO.getModel())){
            ...
            if(promptTokens>=model.getMaxToken()-maxtoken)throw new EventException(EAI00000102);
            return true;
        }
        throw new EventException(EAI00000103);
    }
}
