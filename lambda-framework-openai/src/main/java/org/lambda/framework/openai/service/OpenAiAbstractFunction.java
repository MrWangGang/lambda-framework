...
public interface OpenAiAbstractFunction {
    ...
    public default boolean limitVerify(Long quota,Integer maxtoken,Integer promptTokens) {
        if(quota<=0)throw new EventException(OpaiExceptionEnum.ES_OPAI_016);
        if(maxtoken<=0)throw new EventException(OpaiExceptionEnum.ES_OPAI_017);
        if(quota<=maxtoken)throw new EventException(OpaiExceptionEnum.ES_OPAI_016);
        return true;
    }
    ...
    public default boolean limitVerifyByModel(OpenAiModelEnum model, Long quota, Integer maxtoken, Integer promptTokens) {
        if(model.getModel().equals(OpenAiModelEnum.TURBO.getModel())){
            ...
            if(promptTokens>=model.getMaxToken()-maxtoken)throw new EventException(OpaiExceptionEnum.ES_OPAI_018);
            return true;
        }
        throw new EventException(OpaiExceptionEnum.ES_OPAI_019);
    }
}
